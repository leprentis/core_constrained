...
int main(int argc, char *argv[])
{
	...
	if (system_env != NULL) {
		...
		strcat(minfo.connect_file, "/parameters/antechamber/CONNECT.TPL");
	} else {
	...
	if (system_env != NULL) {
		...
		strcat(pfilename, "/parameters/leap/parm/gaff.dat");
	} else {
		/*    printf("DOCK_HOME environment not set, using gaff.dat in the current directory\n"); */
	...
	if (cindex == 0) {
		if (system_env != NULL) {
			...
			strcat(cfilename, "/parameters/antechamber/ATCOR.DAT");
			cindex = 1;
		}
	}
	if (cindex == 0)
