                    ...
                    for (int x=0; x < growing.size(); ++x){
                        ...
                        if (layer.size() < dn_max_layer_size){
                            ...
                            if (growing[x].mol.rot_bonds <= dn_constraint_rot_bon && 
                                ...
                                } else if (dn_MW_cutoff_type_soft && mw_cutoff(growing[x], evaluate_both)){
                                    layer.push_back( growing[x] );
...
// add an option to GA through all the new fragments you just built at the appropriate place in the code below (no additions or deletions, just cross-overs with occasional mutations) 
                ...
                // Else if there is no dummy remaining...
                    ...
                    if (dn_ga_flag == false){
                        ...
                        if (!dn_normal) {
                            ...
                            fout_molecules << fixed << DELIMITER << setw(STRING_WIDTH) << "TPSA:" << setw(FLOAT_WIDTH) << next_layer[i].mol.tpsa << endl;
                            ...
                            fout_molecules << DELIMITER << setw(STRING_WIDTH) << "num_of_PAINS:" << setw(FLOAT_WIDTH) << next_layer[i].mol.pns << endl; 
                            fout_molecules << DELIMITER << setw(STRING_WIDTH) << "PAINS_names:" << setw(FLOAT_WIDTH) << molpns_name << endl; 
                        }// Descriptors (end)
                        #endif   
            ...
            if (dn_save_all_molecules) {
                ...
                for (unsigned int i = 0; i < rejected.size(); ++i) {
                    ...
                    fout_rejected << fixed << DELIMITER << setw(STRING_WIDTH) << "TPSA:" << setw(FLOAT_WIDTH) << rejected[i].mol.tpsa << endl;
                    ...
                    fout_rejected << DELIMITER << setw(STRING_WIDTH) << "num_of_PAINS:" << setw(FLOAT_WIDTH) << rejected[i].mol.pns << endl; 
                    fout_rejected << DELIMITER << setw(STRING_WIDTH) << "PAINS_names:" << setw(FLOAT_WIDTH) << molpns_name << endl; 
...
bool DN_Build::mw_cutoff( Fragment &temp_molecule, bool evaluate_both ){ // By GDRM, adapted from CPC previous functions
    // Define variables
    float  rand_num{};
    float  rand_num_dec{};
    double excessMW{};
    ...
    // if evaluate_both == false, only evaluate upper boundary
    if (!evaluate_both && temp_molecule.mol.mol_wt > dn_upper_constraint_mol_wt){
    ...
    // if evaluate_both == true and mol.mol_wt > dn_upper_constraint_mol_wt
    } else if (evaluate_both && temp_molecule.mol.mol_wt > dn_upper_constraint_mol_wt){
        rand_num = (rand() % 100 +1) ; // This generates a random number from 1 to 100
        rand_num_dec = rand_num / 100;
        excessMW = temp_molecule.mol.mol_wt - dn_upper_constraint_mol_wt; //How much the growing molecule exceeds cutoff by
        Z_scoreExcess = excessMW / dn_MW_std_dev; // Similar to a Z score (how many std dev the excess mw is from the cutoff)
    ...
    // if evaluate_both == true and mol.mol_wt < dn_lower_constraint_mol_wt
    } else if (evaluate_both && temp_molecule.mol.mol_wt < dn_lower_constraint_mol_wt){
        rand_num = (rand() % 100 +1) ;
        rand_num_dec = rand_num / 100;
    ...
    // if mol.mol_wt < dn_upper_constraint_mol_wt and mol.mol_wt > dn_lower_constraint_mol_wt
    } else if (!evaluate_both && temp_molecule.mol.mol_wt < dn_upper_constraint_mol_wt) { // perhaps redundant [GDRM]
        return true;
    } else {
        return true;
    }
} // End DN_Build::mw_cutoff_general()
