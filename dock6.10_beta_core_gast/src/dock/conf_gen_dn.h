...
class           DN_Build {
  ...
  public:
    ...
    // The variables below don't need to be here; they are not parameters that 
    // define a de novo design experiment.
    /*
    // Section defined by CPC
    double             excessMW;                  //MW differerence between growing molecule and cutoff value
    double             Z_scoreExcess;             //excessMW divided by dn_MW_std_dev
    double             acceptRate;                //Acceptance rate fragment for given Z_scoreExcess
    float              rand_num;                  //random number seeded by time between 1-100
    float              rand_num_dec;              //random number between 0.01 - 1.00, needed a separate variable to work
    */
    ...
    //bool            mw_cutoff( vector <Fragment> &, Fragment &, int); //CPC fucntion
    bool            mw_cutoff( Fragment &, bool ); // GDRM version of function above
