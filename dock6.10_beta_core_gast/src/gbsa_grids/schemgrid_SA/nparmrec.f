...
c**********************************************************
c set up polarity type for each VDW type
      do nvtyp=1,7
        nhp(nvtyp)=1
      end do
      nhp(6)=0
...
c**********************************************************
c add in vdw(nvtyp) (vdw radius of the nth atom)
      read (line, 1002) sra(nvtyp), srb(nvtyp), vdw(nvtyp)
 1002 format (10x, F8.2, 5x, F8.2, 5x, F5.3)


c add in sphere grids for each atom type
      if (vdw(nvtyp).gt.1.e-3) then
        call sphgrid(vdw(nvtyp)+r_probe, nsphgrid(nvtyp), sgcrd)

        do n=1,nsphgrid(nvtyp)
          do i=1,3
            sphgrid_crd(nvtyp,n,i)=sgcrd(n,i)
          end do
        end do
...
c
      ...
      open (unit=12, file='PDBPARM', status='unknown')
      open (unit=13, file='OUTPARM', status='unknown')
c
...
c******************* modified ********************************
                      write (12, 2000) natm, 0, 0.0, 0.0, 0.0, 0.0,
     &                line(31:54)
c************************************************************
                      go to 20
                    endif
...
c************************************************************
c add in vdw(vdwtyp(n)) (vdw radius)
     &vdw(vdwtyp(n)), crg(n), line(31:54)
 2000 format (2I5, 2(1x, F8.2), 1x, F5.3, 1x, F8.3, 1x, A24)
c************************************************************
c
