...
int	putdist( char fname[], MOLECULE_T *mol, char aexp1[], char aexp2[] )
{
  ...
  for( sp1 = mol->m_strands; sp1; sp1 = sp1->s_next ){
    ...
    for( r1 = 0; r1 < sp1->s_nresidues; r1++ ){
      ...
      res1->r_attr |= ( res1->r_attr & AT_SELECT ) ?
	AT_SELECTED : 0;
      for( a1 = 0, ap1 = res1->r_atoms; a1 < res1->r_natoms;
	   a1++, ap1++ ){
	ap1->a_attr &= ~AT_SELECTED;
	ap1->a_attr |= ( ap1->a_attr & AT_SELECT ) ?
	  AT_SELECTED : 0;
      }
    }
  }
	  ...
	  for( a2 = 0; a2 < res2->r_natoms; a2++ ){
	    ...
	    if( AT_SELECT & ap2->a_attr ){
	      fprintf( fp, "%3d %-4s %-4s %3d %-4s %4s %8.3f\n",
		       r1 + 1, res1->r_resname, ap1->a_atomname,
		       r2 + 1, res2->r_resname, ap2->a_atomname,
		       dist( ap1, ap2 ) );
	      cnt++;
	    }
	  }
...
static MOLECULE_T *fgetpdb( FILE *fp, char *options )
{
  ...
  *l_rname = '\0';
  ...
  for( nchains = 0, n_atab = 0; ; ){
    ...
    for( i=strlen( line ); i<80; i++ ) line[i] = ' ';
    line[ 80 ] = '\0';
	...
	aname[i] = temp;
      ...
      if( aname[2] == '*' ) aname[2] = '\'';
      if( !strcmp( aname, "C5M" ) ) strcpy( aname, "C7" );
	...
	if( *np != ' ' )
	 ...
	 will have to be written to pre-scan the entire resiude :-(   */
      if( !strcmp( aname, "O2\'" ) ) res.r_kind = RT_RNA; 
      if( !strcmp( aname, "H2\'1") && res.r_kind == RT_RNA )
	strcpy( aname, "H2\'" );

      /* change Brookhaven nucleic acid residue names to iupuac */
      if( !strcmp( rname, "A" ) ) strcpy( rname, "ADE" );
      if( !strcmp( rname, "C" ) ) strcpy( rname, "CYT" );
      if( !strcmp( rname, "G" ) ) strcpy( rname, "GUA" );
      if( !strcmp( rname, "T" ) ) strcpy( rname, "THY" );
      if( !strcmp( rname, "U" ) ) strcpy( rname, "URA" );
	...
	else r = 1.7;    /* carbon-like radius for others */
	q = 0.0;
	...
	if( n_atab > 0 ){
	  ...
	  res.r_natoms = n_atab;
	  makebonds( &res );
	  addresidue( mol, sname, &res ); initres();
	  n_atab = 0;
	}
...
{	/* temp, will do right soon!	*/
	...
	for( sp = NULL, spl = mol->m_strands; spl; spl = spl->s_next ){
	...
	sp->s_residues =
		(RESIDUE_T **)malloc(sp->s_res_size*sizeof(RESIDUE_T));
  ...
  */
  ...
  for (i=0; i < cifFiles.datablocks[iblock].categories[iCat].numRow; i++) {
    ...
    */
    ...
    rnum = atoi(
		cifFiles.datablocks[iblock].categories[iCat].rows[i].columns[col5]);
	...
	res.r_natoms = n_atab;
	makebonds( &res );
	addresidue( mol, sname, &res ); NAB_initres( &res );
	n_atab = 0;
      }
...
static	void	initres( void )
{
  ...
  if( res.r_resname == NULL ){
    ...
    if( res.r_resname == NULL ){
      fprintf( stderr,
	       "initres: can't allocate res->r_resname.\n" );
      exit( 1 );
    }
  }
  if( res.r_resid == NULL ){
    ...
    if( res.r_resid == NULL ){
      fprintf( stderr,
	       "initres: can't allocate res->r_resid.\n" );
      exit( 1 );
    }
  }
  res.r_natoms = n_atab;
...
static	void	init_atab( void )
{
  ...
  for( ap = atab, a = 0; a < ATAB_SIZE; a++, ap++ ){
    ...
    if( ap->a_atomname == NULL ){
      fprintf( stderr,
	       "init_atab: can't alloc a_atomname.\n" );
      exit( 1 );
    }
    *ap->a_atomname = '\0';
    ap->a_atomtype = NULL;
...
static	void	fputpdb( FILE *fp, MOLECULE_T *mol, char *options )
   ...
   -nobocc:  don't add occupancies and b-factors after the xyz coordinates
    ...
    -brook:  use Broohaven atom/residue names, else use whatever is there
...
*/
{
  ...
  if( options == NULL ) *loptions = '\0';
  else { strncpy( loptions, options, 255 ); loptions[255] = '\0'; }
  ...
  for( ta = 0, tr = 0, sp = mol->m_strands; sp; sp = sp->s_next ){
    ...
    */
	...
	if( !strcmp( rid, "" ) || strstr( loptions, "-tr" ) ){
	  /* id string is empty or "-tr" is set: use residue numbers & chain id on output */
				/* amber numbers are total number of residues	*/
				/* without resetting at chain ids.  Ordinary	*/
				/* rn's begin at 1 for each chain		*/
	  rn = strstr( loptions, "-tr" ) ? tr + 1 : r + 1;
	  if( ta < 100000 ){
	    fprintf( fp,
		     "ATOM  %5d %-4s %-3s %c%4d    %8.3f%8.3f%8.3f",
		     ta, aname, rname,
		     strstr( loptions, "-nocid" ) ? ' ' : cid, rn,
		     ap->a_pos[ 0 ],
		     ap->a_pos[ 1 ],
		     ap->a_pos[ 2 ] );
	  }else{
	...
	} else if( !strstr( loptions, "-nobocc" ) ) {
...
static	void	mk_brook_aname( char aname[], char name[], char rname[] )
{
  ...
  if( strlen( name ) >= 4 && name[3] != ' ' ) aname[0] = name[3];
  aname[1] = name[0];
  aname[2] = name[1];
  aname[3] = name[2];
  if( aname[3] == '\'' ) aname[3] = '*';  /* for nucleic acids */
  aname[4] = '\0';
