...
c
      open (unit=1, file='INRDSL', status='old')
      open (unit=2, file='OUTRDSL', status='new')
c
      read (1, 1000) recfil
 1000 format (A80)
...
c
      ...
      write (2, 1000) grdfil
c   --read desolvation grid calculation method kxr
      read(1, *) idsol
      if (idsol .eq. 1) then 
        write(2,*) 'Occupancy desolvation grid will be generated'
      else if (idsol .eq. 2) then 
        write(2,*) 'Delphi desolvation grid will be generated'        
c--   read receptor solvation energy precalculated by delphi kxr
        read (1, *) rsol_e
        write(2, *) 'pre-calc. receptor solvation energy (kcal/mol):'
        write(2, '(f10.2)') rsol_e
      endif
      close (1)
c
c  --initialize grid
