...
while ( "-" eq substr( $ARGV[0], 0, 1 ) ) {
    ...
    elsif ("n" eq substr( $ARGV[0], 1 ) ) {
        $amberize_options .= ' -n' ;  # Neutralize
        shift ;
    }
    elsif ("s" eq substr( $ARGV[0], 1 ) ) {
        $amberize_options .= ' -s' ;  # Solvate
        shift ;
    }
    else {
...
system( "'/projects2/insite/lauren.prentis/dock6_builds/dock6.10_beta_core_gast'/bin/amberize_receptor $amberize_options $rec_file_prefix 1> amberize_receptor.out 2>&1")
...
for ($lignum = 1; $lignum <= $global_count; $lignum++)
{
    ...
    if ( system( "'/projects2/insite/lauren.prentis/dock6_builds/dock6.10_beta_core_gast'/bin/amberize_ligand $prefix.$lignum $charge_method 1> amberize_ligand.$prefix.$lignum.out 2>&1")
        == 0 )
    {
    ...
    if ( system( "'/projects2/insite/lauren.prentis/dock6_builds/dock6.10_beta_core_gast'/bin/amberize_complex $amberize_options $rec_file_prefix $prefix.$lignum 1> amberize_complex.$prefix.$lignum.out  2>&1")
        == 0 )
    {
...
{
    my $usagedoc = <<EOF;
Usage: $0 [-c] [-i] [-n] [-s] DOCK_ranked_mol2_file Receptor_PDB_file
