...
system( "'/projects2/insite/lauren.prentis/dock6_builds/dock6.10_beta_core_gast'/bin/amberize_receptor $rec_file_prefix 1> amberize_receptor.out 2>&1")
...
for ($lignum = 1; $lignum <= $global_count; $lignum++)
{
    ...
    if ( system( "'/projects2/insite/lauren.prentis/dock6_builds/dock6.10_beta_core_gast'/bin/amberize_complex $rec_file_prefix $prefix.$lignum 1> amberize_complex.$prefix.$lignum.out  2>&1")
        == 0 )
    {
