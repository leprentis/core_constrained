...
system( "'DOCKHOME'/bin/amberize_receptor $amberize_options $rec_file_prefix 1> amberize_receptor.out 2>&1")
...
for ($lignum = 1; $lignum <= $global_count; $lignum++)
{
    ...
    if ( system( "'DOCKHOME'/bin/amberize_complex $amberize_options $rec_file_prefix $prefix.$lignum 1> amberize_complex.$prefix.$lignum.out  2>&1")
        == 0 )
    {
