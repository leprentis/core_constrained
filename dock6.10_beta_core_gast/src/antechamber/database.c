...
void rdef(char *filename)
{
	...
	if (molname_fl  != 0) 
		if(molname_ld < 0) {
			printf("\nMolname_ld must >=0 when the first line of each record is taken as the reference line, exit");
			exit(0);
		}
	if (proc_begin  <= 0) 
		proc_begin = 1;
...
/* read in molname for the first record if molname_fr is applied*/
	if(molname_fr != 0 && process_flag == 1) {
		tmpint1 = molname_fr + molname_ld -1;	
		tmpint2 = 0;
		for(;;) {
...
/* check omitted lines */
		...
		rbegin_line_flag = 0;
		rend_line_flag = 0;
/*get record name if molname_field_index == 1 */
		if (process_flag == 1 && molname_field_index== 1 && strstr(line, field) != 0) {
			if (molname_ld == -5) 
			...
			if (molname_ld == -1) 
				sscanf(&linebak[molname_col], "%s", molname);
			if (molname_ld == 0) 
				sscanf(&line[molname_col], "%s", molname);
			if (molname_ld == 1) {
				pos = ftell(fpin);
...
/*add lines to the benginning of the file*/
				...
				if(process_flag == 1 && separator_type == 0)
					fprintf(fptmp,"%s", line);
			}
...
/*if error happens, stop running*/
		...
		if(process_flag == 1 && separator_line_flag == 0 && rbegin_line_flag == 0 && rend_line_flag == 0)
			fprintf(fptmp,"%s", line);
