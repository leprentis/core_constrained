...
c      read (1, *) spacing
      spacing=grddiv
      write (2, *) 'spacing of spherical grid on each atom:'
      write (2, '(G13.7)') spacing

      read (1, *) r_probe
      write (2, *) 'probe radius:'
      write (2, '(G13.7)') r_probe
...
c************************* modified ***************************
      if (estype .eq. 0) then
        ...
        write (2, *) 'SA terms will be calculated'
      endif
c**************************** END *****************************
c      read (1, *) esfact
c      write (2, '(A31, A15, F6.2)') 'the dielectric function will be',
c     &' multiplied by ', esfact
   75 continue


      read (1, *) r_cutoff
      write (2, *) 'cutoff of inter-atom distance for SAS evaluation:'
      write (2, '(G13.7)') r_cutoff
      r2_cutoff=r_cutoff*r_cutoff


c      read (1, *) cutoffb
c      write (2, *) 'cutoff distance for receptors outside the box:'
c      write (2, *) cutoffb
      grdcutb=int(cutoffb/grddiv + 1.0)

c      read (1, *) cutoff
c      write (2, *) 'cutoff distance for energy calculations:'
c      write (2, *) cutoff
      cutoff = 0
      cutsq=cutoff*cutoff
...
c
c      read (1, *) pcon, ccon
c      write (2, *) 'distances defining bumps with receptor atoms:'
c      write (2, '(A21, F5.2)') 'receptor polar atoms ', pcon
c      write (2, '(A22, F5.2)') 'receptor carbon atoms ', ccon
      pcon=2.3
      ccon=2.8
      pconsq=pcon*pcon
      cconsq=ccon*ccon
...
c      read (1, *) f_scale
      f_scale=4.0
c      write (2, *) 'scale factor in exponent term of f_gb:'
c      write (2, *) f_scale
...
c        call polariz(3, grdcut, grddiv, grdpts, offset, 
c     &grdcuto, natm_total)
        call sas(3, grddiv, grdpts, offset, grdcuto, natm_total)
      endif

      write(2,*) 'total SAS = ', nsas*spacing*spacing
      write(2,*) 'nonpolar SAS = ', nsas_hp*spacing*spacing
      write(2,*) 'polar SAS = ', nsas_pol*spacing*spacing
      write(2,*) 'nonpolar SAS + polar = ', 
     &           (nsas_hp+nsas_pol)*spacing*spacing
